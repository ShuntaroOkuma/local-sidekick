...
#### MVP（Phase 0）: ルール高速判定 + LLM曖昧ケース判定

- 明白なケース（離席・アイドル・高信頼の集中）はルールベースで即判定（0-5ms）
- 曖昧なケース（横向き会話、オンラインMTG中、アプリ内でのだらだら閲覧等）はローカルLLM（Qwen 3B/7B）で判定
- 日次レポートの文章化（Vertex AI）

#### 将来（Phase 1以降）: 時系列コンテキストLLM判定

- 直近60秒分のセンサー時系列（カメラ特徴量 + PC操作パターン）をまとめてLLMに渡し、文脈を考慮した判定を行う
- ルールは離席・アイドルのガードレールのみに縮小し、LLMが主判定エンジンとなる
- 期待される改善点:
  - 「一瞬横を向いた」vs「ずっと横を向いている」の時間的パターン認識
  - MTG中の定期的なカメラ外視線など、行動パターン全体での判定
  - 新しい状態（meeting, break, pair_programming等）のプロンプト追記だけでの追加
  - ユーザーフィードバック（誤判定報告）をfew-shot例として蓄積し、判定精度を継続改善
- 前提条件: より大きなローカルモデル（13B+）またはクラウドLLM（GPT-4o-mini等）の利用
...
- 精度改善（誤検知削減、個人適応、キャリブレーション）
- 状態推定の進化: 時系列コンテキストLLM判定への移行（6.3節参照）
