...
cd /Users/s-ohkuma/code/pw/products/local-sidekick/poc
source .venv/bin/activate
python -m experiment1_embedded.run_text_llama_cpp --duration 300 --interval 5
```
...
- 両プロセスがそれぞれllama_cppモデル（~2GB）をロードするので、合計 ~4GB のメモリを消費します
- カメラ側は5秒間隔、PC側は30秒間隔なので、GPU推論の衝突は少ないはず
- 5分間の通常作業中に、集中→よそ見→離席→戻るなどの状態を混ぜると、両方の結果を比較しやすくなり
  ます
